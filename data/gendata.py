...
class DataGenerator:
    ...
    def parse(self, wkdir, stndir, fthresh=5.0):
        ...
        if not os.path.exists(pwipath):
            self.logger.warning(f"{self.lprefix}Input file not found for {stndir} at {pwipath}. Skipping...")
            return None, None

        if not os.path.exists(pwopath):
            self.logger.warning(f"{self.lprefix}Output file not found for {stndir} at {pwopath}. Skipping...")
            return None, None
        ...
        if forces.any() > fthresh:
            self.logger.warning(f"{self.lprefix}Warning: Large forces detected for {stndir}. Max force magnitude: {np.max(np.linalg.norm(forces, axis=1)):.4f} eV/Å. Skipping this sample.")
            return None, None
        ...
        if not os.path.exists(tb2jpath):
            self.logger.warning(f"{self.lprefix}TB2J output file not found for {stndir} at {tb2jpath}. Skipping TB2J parsing...")
    ...
    def generate(self, datasetdir, rattleidxs):
        ...
        for stnsdir, stnvalues, rattleidxlist in zip(self.stntypes, self.strains, rattleidxs):
            ...
            self.logger.info(f"Processing work directory: {stnsdir}...")
            self.logger.info(f"-" * 50)
            for strain, rattleidx in zip(stnvalues, rattleidxlist):
                ...
                self.logger.info(f"Processing strain (rattled): {stndir}...")
